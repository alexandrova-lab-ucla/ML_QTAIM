...
def score_single(reg, x_train, x_test, y_train, y_test, scale=(1, 0)):
    ...
    plt.plot(
        y_pred_train * scale[0] + scale[1],
        y_train * scale[0] + scale[1],
        "o",
        color="blue",
        markersize=5,
        label="Train",
    ...
    plt.plot(
        y_pred_test * scale[0] + scale[1],
        y_test * scale[0] + scale[1],
        "o",
        color="red",
        label="Validation",
    )
    ...
    plt.ylabel("True Value [kJ/mol]", fontsize=19)
    plt.yticks(fontsize=14)
    plt.xlabel("Predicted Value [kJ/mol]", fontsize=19)
    plt.xticks(fontsize=14)
    name = str(reg)
    plt.title("XGBoost Parity", fontsize=18)
    plt.show()
    plt.tight_layout()
...
def score(reg, x_train, x_test, y_train, y_test, scale=1):
    ...
    mae_test = str(
        mean_absolute_error(reg.predict(x_test) * scale[0], y_test * scale[0])
    )
    ...
    mae_train = str(
        mean_absolute_error(reg.predict(x_train) * scale[0], y_train * scale[0])
    )
    print("MAE train score:   " + str(mae_train))
